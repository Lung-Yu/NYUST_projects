...
//		printf("%dŠ~ %d€ë %d€é", date->tm_year+1900, date->tm_mon+1, date->tm_mday);
    ...
    __android_log_print(ANDROID_LOG_INFO, "C_VAD", "file ready");

    int i, frame = 0, temp, vad, num = 1000000000, n = 0, count = 0, level[12];
    ...
    __android_log_print(ANDROID_LOG_INFO, "C_VAD", "build freqlevel file");

    f_out1 = fopen(freqfile, "w");
    //log = fopen("outlog.txt", "a");
    wb_vad_init(&(vadstate));
    __android_log_print(ANDROID_LOG_INFO, "C_VAD", "freqlevel file open !!");
    while (!feof(fp1)) {
        frame++;
    ...
    fclose(read);
    float ret = avg / i;
    __android_log_print(ANDROID_LOG_INFO, "C_VAD", "calc result");
